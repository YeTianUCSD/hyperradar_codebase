...
def boxes3d_kitti_camera_to_imageboxes(boxes3d, calib, image_shape=None):
    """
    Convert 3D bounding boxes in rectified camera coordinates into 2D image boxes.

    Args:
        boxes3d: (N, 7) array or tensor
                 [x, y, z, l, h, w, r] in rect camera coordinates.
                 Can be either torch.Tensor (CPU/GPU) or numpy.ndarray.
        calib:   Calibration object that provides rect_to_img() method.
        image_shape: (H, W) or [H, W], used to clip boxes to image boundaries.

    Returns:
        boxes2d_image: (N, 4) numpy.ndarray (float32)
                       [x1, y1, x2, y2] in image coordinates.
    """

    # ===== DEBUG: function entry and input type =====
    # Ensure boxes3d is a CPU numpy array
    if isinstance(boxes3d, torch.Tensor):
        boxes3d = boxes3d.detach().cpu().numpy()

    # Convert 3D boxes to their 8 corner points in camera coordinates
    corners3d = boxes3d_to_corners3d_kitti_camera(boxes3d)

    # Project 3D corners to image plane using calibration
    pts_img, _ = calib.rect_to_img(corners3d.reshape(-1, 3))

    # Ensure pts_img is a CPU numpy array
    if isinstance(pts_img, torch.Tensor):
        pts_img = pts_img.detach().cpu().numpy()

    pts_img = np.asarray(pts_img, dtype=np.float32)

    # Reshape to (N, 8, 2): 8 corners per box in image coordinates
    corners_in_image = pts_img.reshape(-1, 8, 2)

    # Compute per-box min/max u,v to get 2D bounding boxes
    min_uv = np.min(corners_in_image, axis=1)  # (N, 2) [u_min, v_min]
    max_uv = np.max(corners_in_image, axis=1)  # (N, 2) [u_max, v_max]
    boxes2d_image = np.concatenate([min_uv, max_uv], axis=1).astype(np.float32)

    # Clip boxes to image boundaries if image_shape is provided
    if image_shape is not None:
        h, w = int(image_shape[0]), int(image_shape[1])

        boxes2d_image[:, 0] = np.clip(boxes2d_image[:, 0], 0, w - 1)  # x1
        boxes2d_image[:, 1] = np.clip(boxes2d_image[:, 1], 0, h - 1)  # y1
        boxes2d_image[:, 2] = np.clip(boxes2d_image[:, 2], 0, w - 1)  # x2
        boxes2d_image[:, 3] = np.clip(boxes2d_image[:, 3], 0, h - 1)  # y2


    # Final 2D boxes (numpy, CPU)
