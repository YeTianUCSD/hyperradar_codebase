...
from ..model_utils.feature_adapter import build_feature_adapter_from_cfg


class AnchorHeadSingleRetrainHDAdapter(AnchorHeadTemplate):
    """
    PointPillars AnchorHeadSingle variant for HD retraining with a lightweight
    zero-init feature adapter.
    ...
    Config knobs (optional; safe defaults if missing):
      ...
      MODEL.DENSE_HEAD.ADAPTER:
        ENABLED: bool
        LOCATION: "cls" | "none" (default = "cls")
        RATIO: bottleneck reduction ratio (default = 32)
        ZERO_INIT: if True, adapter starts as exact identity (default = True)
    """
    ...
    def __init__(
        ...
        # -----------------------------
        # Optional lightweight adapter
        # -----------------------------
        adapter_cfg = self.model_cfg.get('ADAPTER', None)
        self.adapter_enabled = False
        self.adapter_location = "none"
        self.cls_adapter = None
        if adapter_cfg is not None and bool(adapter_cfg.get("ENABLED", False)):
            self.adapter_location = str(adapter_cfg.get("LOCATION", "cls")).lower()
            if self.adapter_location not in ("cls", "none"):
                raise ValueError(f"Unsupported DENSE_HEAD.ADAPTER.LOCATION: {self.adapter_location}")
            if self.adapter_location == "cls":
                self.cls_adapter = build_feature_adapter_from_cfg(adapter_cfg, cls_feat_channels)
                self.adapter_enabled = self.cls_adapter is not None
        ...
        # Retrain-only switch: allow gradients through HD logits during training
        ...
        if hd_cfg is not None and bool(hd_cfg.get("ENABLED", False)):
            ...
            self.hd_train_use_grad = bool(hd_cfg.get("TRAIN_USE_GRAD", True))
    ...
    def forward(self, data_dict):
        ...
        # CLS head (2-layer)
        # -----------------------------
        cls_feat_pre_adapter = self.conv_cls_pre(spatial_features_2d)     # [B, C_cls, H, W]
        cls_feat = cls_feat_pre_adapter
        if self.adapter_enabled and self.cls_adapter is not None:
            cls_feat = self.cls_adapter(cls_feat)
        ...
        if self.adapter_enabled and self.cls_adapter is not None:
            self.forward_ret_dict['cls_feat_pre_adapter'] = cls_feat_pre_adapter
            self.forward_ret_dict['cls_feat_post_adapter'] = cls_feat
