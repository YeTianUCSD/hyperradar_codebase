...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Split val set by contiguous scene chunks: prefix for online adapt, suffix for eval."
    )
    parser.add_argument(
        "--val_txt",
        type=str,
    ...
    parser.add_argument(
        "--stream_name",
        type=str,
        default="val_stream_by_scene",
        help="Name of stream split to create",
    )
    parser.add_argument(
        "--eval_name",
        type=str,
        default="val_eval_by_scene",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Within each contiguous scene chunk, use the first stream_ratio portion for stream",
    )
...
def _split_chunk_prefix_suffix(chunk: List[dict], stream_ratio: float) -> Tuple[List[dict], List[dict]]:
    n = len(chunk)
    if n <= 1:
        raise RuntimeError(
            f"Contiguous chunk {chunk[0]['sample_id']}..{chunk[-1]['sample_id']} has length {n}; "
            "cannot split a scene chunk of length <= 1 into prefix/suffix."
        )
...
def _chunk_report_entry(idx: int, chunk: List[dict], stream_chunk: List[dict], eval_chunk: List[dict]) -> Dict:
    return {
        "chunk_id": int(idx),
...
def main():
    ...
    for idx, chunk in enumerate(chunks):
        stream_chunk, eval_chunk = _split_chunk_prefix_suffix(chunk, stream_ratio=stream_ratio)
        stream_samples.extend(stream_chunk)
        eval_samples.extend(eval_chunk)
        chunk_reports.append(_chunk_report_entry(idx, chunk, stream_chunk, eval_chunk))
    ...
    report = {
        "mode": "scene_prefix_suffix_split",
    ...
    print("[DONE] by-scene split created")
