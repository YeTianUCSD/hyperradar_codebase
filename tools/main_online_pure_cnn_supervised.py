...
from modules.supervised_pure_cnn_runner import SupervisedPureCNNRunner
...
def parse_config():
    parser = argparse.ArgumentParser(description="HyperRadar Pure CNN Supervised Online Adaptation")
    ...
    parser.add_argument("--online_cfg", type=str, required=True,
                        help="Path to pure CNN supervised online yaml config. Expected root key: PURE_CNN_SUPERVISED_ONLINE")

    parser.add_argument("--extra_tag", type=str, default="online_pure_cnn_supervised", help="Output tag")
...
def _build_output_and_logger(args):
    ...
    log_file = output_dir / ('online_pure_cnn_supervised_%s.log' % datetime.datetime.now().strftime('%Y%m%d-%H%M%S'))
    logger = common_utils.create_logger(log_file, rank=cfg.LOCAL_RANK)

    logger.info('**********************Start pure CNN supervised online logging**********************')
...
def _load_online_cfg_file(path: str) -> Dict:
    ...
    online_cfg = raw.get('PURE_CNN_SUPERVISED_ONLINE', raw)
    if not isinstance(online_cfg, dict):
        raise RuntimeError(f'Invalid PURE_CNN_SUPERVISED_ONLINE section type: {type(online_cfg)}')
...
def _save_run_artifacts(output_dir: Path, args, online_cfg: Dict, logger):
    ...
    with open(output_dir / 'online_cfg_resolved.yaml', 'w') as f:
        yaml.safe_dump({'PURE_CNN_SUPERVISED_ONLINE': online_cfg}, f, sort_keys=False)
...
def _load_runner_best_state_into_model(runner: SupervisedPureCNNRunner, logger):
...
def main():
    ...
    runner = SupervisedPureCNNRunner(
        model=model,
        optimizer=optimizer,
        ...
        state_save_prefix='supervised_pure_cnn',
    )

    logger.info('[ONLINE] pure CNN supervised runner start')
    runner.run(max_steps=args.max_steps)
    logger.info('[ONLINE] pure CNN supervised runner finished')
    ...
    logger.info('**********************End pure CNN supervised online**********************')
