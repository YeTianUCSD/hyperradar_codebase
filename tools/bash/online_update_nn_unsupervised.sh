...
OUT_ROOT=/home/code/hyperradar/hyperradar_codebase/output/kitti_models/online_cnn_unsupervised
RUN_TAG=online_v1_cnn_clsout_only_retraincfg_ckpt50
LOG=$OUT_ROOT/$RUN_TAG/run_$(date +%F_%H%M%S).log

mkdir -p "$OUT_ROOT/$RUN_TAG"

python -u main_online_cnn.py \
  ...
  --online_cfg cfgs/online/pointpillar_vod_cnn_online.yaml \
