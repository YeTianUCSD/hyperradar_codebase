...
class OnlineCNNConfig:
    ...
    pseudo_logits_source: str = "origin"  # final | origin | hd
...
class OnlineCNNRunner:
    """
    Unsupervised online CNN classifier adaptation runner.

    Design:
    - Force detector classification to use baseline CNN logits.
    ...
    - model.dense_head has conv_cls_out and hd_cls_feat is written into batch_dict.
    ...
    def __init__(
        ...
        *,
        ...
        state_save_prefix: str = "online_cnn",
        use_amp: bool = False,
    ):
        self.model = model
        ...
        if self.dense_head is None:
            raise RuntimeError("OnlineCNNRunner requires model.dense_head.")
        if not hasattr(self.dense_head, "conv_cls_out"):
            raise RuntimeError("OnlineCNNRunner requires dense_head.conv_cls_out.")
    ...
    def run(self, *, max_steps: int = -1):
        ...
        self._log("[ONLINE-CNN] start")
        self._log(
            f"[ONLINE-CNN] cfg: pseudo_logits_source={self.cfg.pseudo_logits_source}, "
        ...
        self._log(f"[ONLINE-CNN] finished | steps={self.step_idx} updates={self.update_idx} elapsed={time.time()-t0:.1f}s")
    ...
    def _compute_teacher_logits(self, batch_dict: Dict[str, Any]) -> torch.Tensor:
        ...
        if feat_map is None:
            raise RuntimeError("Expected batch_dict['hd_cls_feat'] to compute teacher CNN logits.")
    ...
    def _save_state(self, tag: str):
        ...
        payload["runner_type"] = "online_cnn_runner"
    ...
    def _export_best_model_payload(self):
        if self.output_dir is None or self.best_state is None:
            return
        path = self.output_dir / "best_online_cnn_model.pth"
    ...
    @staticmethod
    def _build_cfg(cfg_in: Dict[str, Any]) -> OnlineCNNConfig:
        cfg = OnlineCNNConfig()
