...
@dataclass
class OnlinePureCNNBackboneClsConfig:
...
class OnlinePureCNNBackboneClsRunner:
    """
    Unsupervised pure-CNN backbone + classification-branch adaptation runner.

    Design:
    - Use a NO-HD detector and adapt backbone_2d plus the CNN classification branch.
    - Keep VFE, BEV mapping, box head, and direction head frozen.
    ...
    def __init__(
        ...
        *,
        ...
        state_save_prefix: str = "online_pure_cnn_backbone_cls",
        use_amp: bool = False,
    ):
        self.model = model
        ...
        if self.dense_head is None:
            raise RuntimeError("OnlinePureCNNBackboneClsRunner requires model.dense_head.")
        if not hasattr(self.dense_head, "conv_cls_out"):
            raise RuntimeError("OnlinePureCNNBackboneClsRunner requires dense_head.conv_cls_out.")
        if not hasattr(self.dense_head, "conv_cls_pre"):
            raise RuntimeError("OnlinePureCNNBackboneClsRunner requires dense_head.conv_cls_pre.")
    ...
    def run(self, *, max_steps: int = -1):
        ...
        self._log("[ONLINE-PURE-CNN-BACKBONE-CLS] start")
        self._log(
            f"[ONLINE-PURE-CNN-BACKBONE-CLS] cfg: pseudo_logits_source={self.cfg.pseudo_logits_source}, "
        ...
        self._log(f"[ONLINE-PURE-CNN-BACKBONE-CLS] finished | steps={self.step_idx} updates={self.update_idx} elapsed={time.time()-t0:.1f}s")
    ...
    def _save_state(self, tag: str):
        ...
        payload["runner_type"] = "online_pure_cnn_backbone_cls_runner"
    ...
    def _export_best_model_payload(self):
        if self.output_dir is None or self.best_state is None:
            return
        path = self.output_dir / "best_online_pure_cnn_backbone_cls_model.pth"
    ...
    @staticmethod
    def _build_cfg(cfg_in: Dict[str, Any]) -> OnlinePureCNNBackboneClsConfig:
        cfg = OnlinePureCNNBackboneClsConfig()
