...
from supervised_online_hd_adapter_runner import SupervisedOnlineHDAdapterRunner
...
def parse_config():
    parser = argparse.ArgumentParser(description="HyperRadar Supervised Online HD Adaptation with Adapter")
    ...
    parser.add_argument("--train_adapter", type=int, choices=[0, 1], default=None)
    parser.add_argument("--update_hd_memory", type=int, choices=[0, 1], default=None)
    parser.add_argument("--adapter_optimizer", type=str, choices=["adamw", "adam"], default=None)
    parser.add_argument("--adapter_lr", type=float, default=None)
    parser.add_argument("--adapter_weight_decay", type=float, default=None)
    parser.add_argument("--adapter_loss_weight", type=float, default=None)
    parser.add_argument("--adapter_feature_reg_weight", type=float, default=None)
    parser.add_argument("--adapter_proto_align_weight", type=float, default=None)
    parser.add_argument("--adapter_grad_clip", type=float, default=None)
    parser.add_argument("--adapter_max_loss", type=float, default=None)
    parser.add_argument("--adapter_max_grad_norm_skip", type=float, default=None)
    parser.add_argument("--adapter_use_negatives", type=int, choices=[0, 1], default=None)
    parser.add_argument("--adapter_max_pos_per_batch", type=int, default=None)
    parser.add_argument("--adapter_max_neg_per_batch", type=int, default=None)
    parser.add_argument("--adapter_min_pos_per_batch", type=int, default=None)
    parser.add_argument("--adapter_log_every_n_batches", type=int, default=None)
...
def _build_supervised_online_cfg_dict(args) -> Dict:
    mapping = {
        ...
        'adapter_feature_reg_weight': args.adapter_feature_reg_weight,
        'adapter_proto_align_weight': args.adapter_proto_align_weight,
        'adapter_grad_clip': args.adapter_grad_clip,
        'adapter_max_loss': args.adapter_max_loss,
        'adapter_max_grad_norm_skip': args.adapter_max_grad_norm_skip,
        'adapter_max_pos_per_batch': args.adapter_max_pos_per_batch,
        'adapter_max_neg_per_batch': args.adapter_max_neg_per_batch,
        'adapter_min_pos_per_batch': args.adapter_min_pos_per_batch,
        'adapter_log_every_n_batches': args.adapter_log_every_n_batches,
    ...
    if args.train_adapter is not None:
        mapping['train_adapter'] = bool(args.train_adapter)
    if args.update_hd_memory is not None:
        mapping['update_hd_memory'] = bool(args.update_hd_memory)
    if args.adapter_use_negatives is not None:
        mapping['adapter_use_negatives'] = bool(args.adapter_use_negatives)
...
def _load_runner_best_state_into_model(runner: SupervisedOnlineHDAdapterRunner, logger):
    ...
    adapter_state = best_state.get('adapter_state', None)
    if isinstance(adapter_state, dict) and getattr(runner, 'cls_adapter', None) is not None:
        runner.cls_adapter.load_state_dict(adapter_state, strict=False)
        logger.info('[BEST] loaded runner.best_state adapter into model for final evaluation')
...
def main():
    ...
    runner = SupervisedOnlineHDAdapterRunner(
        model=model,
        ...
        state_save_prefix='supervised_online_hd_adapter',
    )
