...
class SupervisedOnlineHDConfig:
    ...
    adapter_betas: Tuple[float, float] = (0.9, 0.999)
    adapter_loss_weight: float = 1.0
    adapter_feature_reg_weight: float = 0.01
    adapter_proto_align_weight: float = 0.1
    adapter_grad_clip: float = 1.0
    adapter_max_loss: float = 5.0
    adapter_max_grad_norm_skip: float = 10.0
    adapter_use_negatives: bool = True
    adapter_max_pos_per_batch: int = 4096
    adapter_max_neg_per_batch: int = 4096
    adapter_min_pos_per_batch: int = 1
    adapter_log_every_n_batches: int = 10
    ...
    experiment_note: str = "supervised_stream_hd_adapter"


class SupervisedOnlineHDAdapterRunner:
    """
    Supervised streaming HD memory updater for the adapter pipeline.
    ...
    - First-stage adapter pipeline keeps adapter training disabled and updates
      hd_core.memory from positive anchor features and labels.
    ...
    def __init__(
        ...
        *,
        ...
        if not self.cfg.gt_only:
            raise RuntimeError("SupervisedOnlineHDAdapterRunner currently supports only gt_only=True.")
        ...
        if self.dense_head is None:
            raise RuntimeError("SupervisedOnlineHDAdapterRunner requires model.dense_head.")

        self.hd_core = getattr(self.dense_head, "hd_core", None)
        if self.hd_core is None:
            raise RuntimeError("SupervisedOnlineHDAdapterRunner requires dense_head.hd_core.")

        self.cls_adapter = getattr(self.dense_head, "cls_adapter", None)
        self.adapter_enabled = bool(getattr(self.dense_head, "adapter_enabled", False))
        self.adapter_optimizer: Optional[torch.optim.Optimizer] = None
        self.adapter_trainable_params = 0
        self._configure_adapter_training()
    ...
    def _configure_adapter_training(self):
        self.model.requires_grad_(False)

        if self.cls_adapter is None:
            if self.cfg.train_adapter:
                raise RuntimeError("train_adapter=True requires dense_head.cls_adapter, but it is missing.")
            return

        if self.cfg.train_adapter and not self.adapter_enabled:
            raise RuntimeError("train_adapter=True requires dense_head.adapter_enabled=True.")

        self.cls_adapter.requires_grad_(bool(self.cfg.train_adapter))
        if not self.cfg.train_adapter:
            return

        params = [p for p in self.cls_adapter.parameters() if p.requires_grad]
        if not params:
            raise RuntimeError("No trainable parameters found in dense_head.cls_adapter.")

        self.adapter_trainable_params = int(sum(p.numel() for p in params))
        opt_name = str(self.cfg.adapter_optimizer).lower()
        if opt_name == "adamw":
            self.adapter_optimizer = torch.optim.AdamW(
                params,
                lr=float(self.cfg.adapter_lr),
                weight_decay=float(self.cfg.adapter_weight_decay),
                betas=tuple(float(x) for x in self.cfg.adapter_betas),
            )
        elif opt_name == "adam":
            ...
            raise RuntimeError(f"Unsupported adapter_optimizer={self.cfg.adapter_optimizer}")
    ...
    def run(self, *, max_steps: int = -1):
        ...
        self._log(
            f"[ADAPTER] enabled={self.adapter_enabled}, present={self.cls_adapter is not None}, "
            f"train_adapter={self.cfg.train_adapter}, update_hd_memory={self.cfg.update_hd_memory}"
        )
        if self.cfg.train_adapter:
            self._log(
                f"[ADAPTER_OPT] optimizer={self.cfg.adapter_optimizer} lr={self.cfg.adapter_lr:.6g} "
                f"weight_decay={self.cfg.adapter_weight_decay:.6g} "
                f"trainable_params={self.adapter_trainable_params}"
            )
        ...
        for batch in self.stream_loader:
            ...
            self.step_idx += 1
            adapter_stats = self._run_adapter_train_step(batch)
            step = self._process_one_step(batch)
            self._accumulate_pending(step)

            if adapter_stats is not None and self.step_idx % self.cfg.adapter_log_every_n_batches == 0:
                self._log(
                    f"[ADAPTER_STEP {self.step_idx}] total={adapter_stats['loss']:.6f} "
                    f"cls={adapter_stats['cls_loss']:.6f} feat_reg={adapter_stats['feature_reg_loss']:.6f} "
                    f"proto={adapter_stats['proto_align_loss']:.6f} "
                    f"pos={adapter_stats['num_pos']} neg={adapter_stats['num_neg']} "
                    f"grad_norm={adapter_stats['grad_norm']:.6f} skipped={adapter_stats['skipped']} "
                    f"reason={adapter_stats['skip_reason']}"
                )
    ...
    def _run_adapter_train_step(self, batch_dict: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        if not self.cfg.train_adapter:
            return None
        if self.adapter_optimizer is None or self.cls_adapter is None:
            raise RuntimeError("Adapter training is enabled, but optimizer/adapter is not initialized.")

        load_data_to_gpu(batch_dict)
        self.model.eval()
        self.cls_adapter.train()
        self.adapter_optimizer.zero_grad(set_to_none=True)

        with torch.enable_grad():
            _pred, _ret = self.model(batch_dict)
            loss, stats = self._compute_adapter_cls_loss()

        if loss is None:
            self.adapter_optimizer.zero_grad(set_to_none=True)
            self.cls_adapter.eval()
            self._clear_forward_outputs(batch_dict)
            return stats
        if not torch.isfinite(loss).all():
            ...
            return stats
        if float(self.cfg.adapter_max_loss) > 0 and float(loss.detach().item()) > float(self.cfg.adapter_max_loss):
            self.adapter_optimizer.zero_grad(set_to_none=True)
            self.cls_adapter.eval()
            self._clear_forward_outputs(batch_dict)
            stats["skipped"] = True
            stats["skip_reason"] = "loss_gate"
            return stats

        loss.backward()
        grad_norm = self._clip_or_measure_adapter_grad_norm()
        if float(self.cfg.adapter_max_grad_norm_skip) > 0 and grad_norm > float(self.cfg.adapter_max_grad_norm_skip):
            self.adapter_optimizer.zero_grad(set_to_none=True)
            self.cls_adapter.eval()
            self._clear_forward_outputs(batch_dict)
            stats["grad_norm"] = float(grad_norm)
            stats["skipped"] = True
            stats["skip_reason"] = "grad_gate"
            return stats
    ...
    def _compute_adapter_cls_loss(self) -> Tuple[Optional[torch.Tensor], Dict[str, Any]]:
        forward_ret = self.dense_head.forward_ret_dict
        cls_hwk = forward_ret.get("cls_preds_origin", None)
        if cls_hwk is None:
            raise RuntimeError(
                "Adapter training requires dense_head.forward_ret_dict['cls_preds_origin']. "
                "Set MODEL.DENSE_HEAD.HD.DEBUG_SAVE_LOGITS=True in the detector cfg."
            )
        box_cls_labels = forward_ret.get("box_cls_labels", None)
        if box_cls_labels is None:
            raise RuntimeError(
                "Adapter training requires dense_head.forward_ret_dict['box_cls_labels']. "
                "Set MODEL.DENSE_HEAD.HD.ASSIGN_TARGETS_IN_EVAL=True or run target assignment."
            )
        if cls_hwk.dim() != 4:
            raise RuntimeError(f"Unexpected cls_preds_origin shape: {tuple(cls_hwk.shape)}")
        ...
        K = self.num_classes
        if AK != A * K:
            raise RuntimeError(f"Unexpected cls_preds_origin last dim: got {AK}, expected {A*K}")

        cls_preds = cls_hwk.reshape(B, H * W * A, K).float()
        labels = box_cls_labels.long()
        if labels.shape[0] != B or labels.shape[1] != H * W * A:
            raise RuntimeError(
                f"box_cls_labels shape {tuple(labels.shape)} does not match cls predictions {(B, H * W * A)}"
            )

        selected = torch.zeros_like(labels, dtype=torch.bool)
        total_pos = 0
        total_neg = 0
        max_pos = int(self.cfg.adapter_max_pos_per_batch)
        max_neg = int(self.cfg.adapter_max_neg_per_batch)

        for b in range(B):
            pos_idx = torch.nonzero(labels[b] > 0, as_tuple=False).squeeze(1)
            neg_idx = torch.nonzero(labels[b] == 0, as_tuple=False).squeeze(1)

            if max_pos > 0 and pos_idx.numel() > max_pos:
                perm = torch.randperm(pos_idx.numel(), generator=self._rng).to(pos_idx.device)
                pos_idx = pos_idx[perm[:max_pos]]
            if not self.cfg.adapter_use_negatives:
                neg_idx = neg_idx[:0]
            elif max_neg > 0 and neg_idx.numel() > max_neg:
                perm = torch.randperm(neg_idx.numel(), generator=self._rng).to(neg_idx.device)
                neg_idx = neg_idx[perm[:max_neg]]

            if pos_idx.numel() > 0:
                selected[b, pos_idx] = True
                total_pos += int(pos_idx.numel())
            if neg_idx.numel() > 0:
                selected[b, neg_idx] = True
        ...
        if total_pos < int(self.cfg.adapter_min_pos_per_batch) or int(selected.sum().item()) == 0:
            stats["skipped"] = True
            stats["skip_reason"] = "not_enough_pos"
            return None, stats

        cared = labels >= 0
        cls_targets = labels * cared.type_as(labels)
        cls_targets = torch.clamp(cls_targets, min=0, max=K)
        one_hot_targets = torch.zeros(
            *list(cls_targets.shape), K + 1, dtype=cls_preds.dtype, device=cls_preds.device
        )
        one_hot_targets.scatter_(-1, cls_targets.unsqueeze(dim=-1), 1.0)
        one_hot_targets = one_hot_targets[..., 1:]

        pos_per_batch = (labels > 0).sum(dim=1, keepdim=True).float().clamp(min=1.0)
        cls_weights = selected.float() / pos_per_batch

        cls_loss_src = self.dense_head.cls_loss_func(cls_preds, one_hot_targets, weights=cls_weights)
        cls_loss = cls_loss_src.sum() / max(B, 1)
        cls_loss = cls_loss * float(self.dense_head.model_cfg.LOSS_CONFIG.LOSS_WEIGHTS["cls_weight"])
        cls_loss = cls_loss * float(self.cfg.adapter_loss_weight)

        feature_reg_loss = cls_loss.new_zeros(())
        if float(self.cfg.adapter_feature_reg_weight) > 0:
            feat_pre = forward_ret.get("cls_feat_pre_adapter", None)
            feat_post = forward_ret.get("cls_feat_post_adapter", None)
            if feat_pre is None or feat_post is None:
                raise RuntimeError(
                    "adapter_feature_reg_weight > 0 requires cls_feat_pre_adapter and cls_feat_post_adapter."
                )
            feature_reg_loss = (feat_post.float() - feat_pre.detach().float()).pow(2).mean()

        proto_align_loss = cls_loss.new_zeros(())
        if float(self.cfg.adapter_proto_align_weight) > 0:
            feat_post = forward_ret.get("cls_feat_post_adapter", None)
            if feat_post is None:
                raise RuntimeError("adapter_proto_align_weight > 0 requires cls_feat_post_adapter.")
            proto_align_loss = self._compute_adapter_proto_align_loss(
                feat_post=feat_post,
                labels=labels,
                selected=selected,
            )
        ...
        total_loss = (
            cls_loss
            + float(self.cfg.adapter_feature_reg_weight) * feature_reg_loss
            + float(self.cfg.adapter_proto_align_weight) * proto_align_loss
        )
        stats["loss"] = float(total_loss.detach().item())
        stats["cls_loss"] = float(cls_loss.detach().item())
        stats["feature_reg_loss"] = float(feature_reg_loss.detach().item())
        stats["proto_align_loss"] = float(proto_align_loss.detach().item())
        return total_loss, stats

    def _compute_adapter_proto_align_loss(
        self,
        *,
        ...
        if feat_post.dim() != 4:
            raise RuntimeError(f"Unexpected cls_feat_post_adapter shape: {tuple(feat_post.shape)}")

        B, C, H, W = feat_post.shape
        A = self.num_anchors_per_loc
        expected_n = H * W * A
        if labels.shape != (B, expected_n):
            raise RuntimeError(
                f"labels shape {tuple(labels.shape)} does not match adapter feature anchors {(B, expected_n)}"
            )

        feat_cell = feat_post.permute(0, 2, 3, 1).contiguous().view(B, H * W, C)
        feat_parts: List[torch.Tensor] = []
        label_parts: List[torch.Tensor] = []
        anchor_parts: List[torch.Tensor] = []

        for b in range(B):
            pos_idx = torch.nonzero(selected[b] & (labels[b] > 0), as_tuple=False).squeeze(1)
            if pos_idx.numel() == 0:
                continue
            cell_idx = torch.div(pos_idx, A, rounding_mode="floor")
            anchor_ids = torch.remainder(pos_idx, A)
            feat_parts.append(feat_cell[b][cell_idx])
            label_parts.append(labels[b][pos_idx] - 1)
            anchor_parts.append(anchor_ids)

        if not feat_parts:
            return feat_post.new_zeros(())

        feat_sel = torch.cat(feat_parts, dim=0)
        labels_sel = torch.cat(label_parts, dim=0).long()
        anchor_ids = torch.cat(anchor_parts, dim=0).long()

        feat_sel = self.hd_core.inject_anchor_context(
            feat_mid=feat_sel,
            anchor_ids=anchor_ids,
            num_anchors=A,
        )
        logits_hd, _ = self.hd_core.compute_hd_logits(feat_sel)
        return F.cross_entropy(logits_hd.float(), labels_sel)

    def _clip_or_measure_adapter_grad_norm(self) -> float:
        params = [p for p in self.cls_adapter.parameters() if p.requires_grad and p.grad is not None]
        if not params:
            return 0.0
        if float(self.cfg.adapter_grad_clip) > 0:
            grad_norm = torch.nn.utils.clip_grad_norm_(params, max_norm=float(self.cfg.adapter_grad_clip))
            return float(grad_norm.detach().item() if torch.is_tensor(grad_norm) else grad_norm)
    ...
    @staticmethod
    def _clear_forward_outputs(batch_dict: Dict[str, Any]):
        for key in (
            "hd_cls_feat",
            "hd_bev_feat",
            "hd_shape_bev",
            "hd_shape_cls",
            "hd_num_anchors_per_loc",
            "hd_num_classes",
            "hd_feat_source",
            "batch_cls_preds",
            "batch_box_preds",
    ...
    def _maybe_apply_pending_update(self, reason: str, force: bool = False):
        if not self.cfg.update_hd_memory:
            self._reset_pending()
            return
    ...
    def _snapshot_state(self) -> Dict[str, Any]:
        ...
        if self.cls_adapter is not None:
            adapter_state = {}
            for k, v in self.cls_adapter.state_dict().items():
                if torch.is_tensor(v):
                    adapter_state[k] = v.detach().cpu().clone()
                else:
                    adapter_state[k] = copy.deepcopy(v)
    ...
    def _save_state(self, tag: str):
        ...
        payload = OnlineStateIO.build_payload(
            ...
            extra={
                "runner_type": "supervised_online_hd_adapter_runner",
                "experiment_note": self.cfg.experiment_note,
                "metric_key": self.cfg.metric_key,
                "adapter_state": adapter_state,
                "adapter_trainable_params": int(self.adapter_trainable_params),
            },
        )
    ...
    def _export_best_memory_payload(self):
        ...
        payload: Dict[str, Any] = {
            ...
            "meta": {
                "source": "supervised_online_hd_adapter_runner.best_state",
                "best_metric": float(self.best_metric),
    ...
    def _build_cfg(cfg_in: Dict[str, Any]) -> SupervisedOnlineHDConfig:
        ...
        cfg.train_adapter = bool(cfg.train_adapter)
        cfg.update_hd_memory = bool(cfg.update_hd_memory)
        cfg.adapter_optimizer = str(cfg.adapter_optimizer).lower()
        cfg.adapter_lr = float(max(0.0, cfg.adapter_lr))
        cfg.adapter_weight_decay = float(max(0.0, cfg.adapter_weight_decay))
        if isinstance(cfg.adapter_betas, (list, tuple)) and len(cfg.adapter_betas) == 2:
            cfg.adapter_betas = (float(cfg.adapter_betas[0]), float(cfg.adapter_betas[1]))
        else:
            cfg.adapter_betas = (0.9, 0.999)
        cfg.adapter_loss_weight = float(max(0.0, cfg.adapter_loss_weight))
        cfg.adapter_feature_reg_weight = float(max(0.0, cfg.adapter_feature_reg_weight))
        cfg.adapter_proto_align_weight = float(max(0.0, cfg.adapter_proto_align_weight))
        cfg.adapter_grad_clip = float(max(0.0, cfg.adapter_grad_clip))
        cfg.adapter_max_loss = float(max(0.0, cfg.adapter_max_loss))
        cfg.adapter_max_grad_norm_skip = float(max(0.0, cfg.adapter_max_grad_norm_skip))
        cfg.adapter_use_negatives = bool(cfg.adapter_use_negatives)
        cfg.adapter_max_pos_per_batch = int(max(0, cfg.adapter_max_pos_per_batch))
        cfg.adapter_max_neg_per_batch = int(max(0, cfg.adapter_max_neg_per_batch))
        cfg.adapter_min_pos_per_batch = int(max(0, cfg.adapter_min_pos_per_batch))
        cfg.adapter_log_every_n_batches = int(max(1, cfg.adapter_log_every_n_batches))
