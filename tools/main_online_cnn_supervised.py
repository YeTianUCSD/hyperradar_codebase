...
from modules.supervised_online_cnn_runner import SupervisedOnlineCNNRunner
...
def parse_config():
    parser = argparse.ArgumentParser(description="HyperRadar Supervised Online CNN Head Adaptation")
    ...
    parser.add_argument("--online_cfg", type=str, required=True,
                        help="Path to CNN supervised online yaml config")

    parser.add_argument("--extra_tag", type=str, default="online_cnn_supervised", help="Output tag")
...
def _build_output_and_logger(args):
    ...
    logger.info('**********************Start supervised online CNN logging**********************')
...
def _load_cnn_online_cfg_file(path: str) -> Dict:
    with open(path, 'r') as f:
        raw = yaml.safe_load(f) or {}
    if not isinstance(raw, dict):
        raise RuntimeError(f'Invalid online cfg file: expected dict root, got {type(raw)}')
    online_cfg = raw.get('CNN_SUPERVISED_ONLINE', raw.get('SUPERVISED_ONLINE', raw))
    if not isinstance(online_cfg, dict):
        raise RuntimeError(f'Invalid CNN_SUPERVISED_ONLINE section type: {type(online_cfg)}')
    return dict(online_cfg)


def _build_cnn_online_cfg_dict(args) -> Dict:
    mapping = {
    ...
    if args.save_best_model is not None:
        mapping['save_best_model'] = bool(args.save_best_model)
    if args.save_last_model is not None:
        mapping['save_last_model'] = bool(args.save_last_model)
...
def _save_run_artifacts(output_dir: Path, args, online_cfg: Dict, logger):
    ...
    with open(output_dir / 'online_cfg_resolved.yaml', 'w') as f:
        yaml.safe_dump({'CNN_SUPERVISED_ONLINE': online_cfg}, f, sort_keys=False)
...
def _build_eval_fn(args, logger, output_dir: Path, online_cfg: Dict, eval_batch_size: int):
    ...
    def _run_eval(eval_model, eval_loader, *, fast_recall_only: bool, eval_name: str):
        ...
        tb_dict = eval_utils.eval_one_epoch(
            ...
            eval_model,
            eval_loader,
            epoch_id=f'sup_online_cnn_{eval_counter["n"]:04d}',
            logger=logger,
            dist_test=False,
...
def _load_runner_best_state_into_model(runner: SupervisedOnlineCNNRunner, logger):
...
def _freeze_everything_except_cnn_head(model, logger):
    total = 0
    trainable = 0
...
def _build_optimizer_for_cnn_head(model, online_cfg: Dict, logger):
    ...
    logger.info(
        f'[OPTIM] optimizer={opt_name} lr={lr} weight_decay={weight_decay} momentum={momentum}'
    )
    return optimizer
...
def main():
    ...
    _freeze_everything_except_cnn_head(model, logger)
    optimizer = _build_optimizer_for_cnn_head(model, online_cfg, logger)

    eval_fn, run_eval = _build_eval_fn(args, logger, output_dir, online_cfg, eval_bs)

    runner = SupervisedOnlineCNNRunner(
        model=model,
        optimizer=optimizer,
        ...
        state_save_prefix='supervised_online_cnn',
    )

    logger.info('[ONLINE] supervised CNN runner start')
    runner.run(max_steps=args.max_steps)
    logger.info('[ONLINE] supervised CNN runner finished')
    ...
    logger.info('**********************End supervised online CNN**********************')
