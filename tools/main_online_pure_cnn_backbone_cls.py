...
from modules.online_pure_cnn_backbone_cls_runner import OnlinePureCNNBackboneClsRunner
...
def parse_config():
    parser = argparse.ArgumentParser(description="HyperRadar Pure CNN Backbone+Cls Unsupervised Online Adaptation")
    ...
    parser.add_argument("--extra_tag", type=str, default="online_pure_cnn_backbone_cls_unsupervised", help="Output tag")
    ...
    parser.add_argument("--lr_backbone_2d", type=float, default=None)
    parser.add_argument("--lr_conv_cls_pre", type=float, default=None)
    parser.add_argument("--lr_conv_cls_out", type=float, default=None)
...
def _build_output_and_logger(args):
    ...
    log_file = output_dir / ('online_pure_cnn_backbone_cls_%s.log' % datetime.datetime.now().strftime('%Y%m%d-%H%M%S'))
    logger = common_utils.create_logger(log_file, rank=cfg.LOCAL_RANK)

    logger.info('**********************Start pure CNN backbone+cls unsupervised online logging**********************')
...
def _build_online_cfg_dict(args) -> Dict:
    mapping = {
        ...
        'lr': args.lr,
        'lr_backbone_2d': args.lr_backbone_2d,
        'lr_conv_cls_pre': args.lr_conv_cls_pre,
        'lr_conv_cls_out': args.lr_conv_cls_out,
...
def _build_optimizer(model, online_cfg: Dict, logger):
    ...
    lr_overrides = {
        'backbone_2d': online_cfg.get('lr_backbone_2d', None),
        'dense_head.conv_cls_pre': online_cfg.get('lr_conv_cls_pre', None),
        'dense_head.conv_cls_out': online_cfg.get('lr_conv_cls_out', None),
    }
    ...
    if lr_overrides:
        ...
        for name, param in named_trainable:
            group_lr = lr
            group_tag = 'default'
            for prefix, override_lr in lr_overrides.items():
                if name.startswith(prefix):
                    group_lr = override_lr
                    group_tag = prefix
                    break
            key = (group_tag, group_lr)
            buckets.setdefault(key, []).append(param)
            bucket_names.setdefault(key, []).append(name)

        trainable_params = []
        for (tag, group_lr), params in buckets.items():
            trainable_params.append({'params': params, 'lr': group_lr, 'weight_decay': weight_decay})
            n_param = sum(p.numel() for p in params)
            logger.info(f'[OPTIM] param_group={tag} lr={group_lr} params={n_param}')
            for name in bucket_names[(tag, group_lr)]:
                logger.info(f'[OPTIM]   {name}')
    else:
...
def _load_runner_best_state_into_model(runner: OnlinePureCNNBackboneClsRunner, logger):
...
def main():
    ...
    if bool(online_cfg.get('use_teacher', False)):
        raise RuntimeError(
            'Backbone+cls unsupervised update intentionally disables use_teacher. '
            'The lightweight teacher only tracks conv_cls_out, while this experiment '
            'also updates backbone_2d and conv_cls_pre.'
        )
    ...
    runner = OnlinePureCNNBackboneClsRunner(
        model=model,
        optimizer=optimizer,
        ...
        state_save_prefix='online_pure_cnn_backbone_cls',
        use_amp=bool(args.use_amp),
    )

    logger.info('[ONLINE] pure CNN backbone+cls unsupervised runner start')
    runner.run(max_steps=args.max_steps)
    logger.info('[ONLINE] pure CNN backbone+cls unsupervised runner finished')
    ...
    logger.info('**********************End pure CNN backbone+cls unsupervised online**********************')
